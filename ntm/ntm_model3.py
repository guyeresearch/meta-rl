...
# 2. feed memory average to controller out
...
class NTM_Head(nn.Module):
    ...
    def __init__(self,p):
        ...
        # k + beta + g + s + gamma
        read_repr_size = self.M + 1 + 1 + len(p.shift_vec) + 1
        self.read_layer = nn.Linear(p.ctrl.output_size + self.M,
                                    read_repr_size)
        
        
        # k + beta + g + s + gamma + e + a
        write_repr_size = self.M + 1 + 1 + len(p.shift_vec) + 1 + self.M*2
        self.write_layer = nn.Linear(p.ctrl.output_size + self.M,
                                     write_repr_size)
    ...
    def _split(self,represent, flag='r'):
        # flag will be either 'r' or 'w'
        M = self.M
        rep = represent
        k = rep[:,:self.M]
        beta = rep[:,[self.M]] # maintain batch * 1 shape
        g = rep[:,[self.M+1]] # maintian batch * 1 shape
        s = rep[:,(self.M+2):(self.M+2+self.shift.size)]
        gamma = rep[:,[self.M+2+self.shift.size]] # maintian batch * 1 shape
        if flag == 'r':
    ...
    def forward(self,x):
        ctrl_out = self.ctrl(x) # batch * output_size
        mem_avg = self.mem.avg() # batch * M
        comb = torch.cat((mem_avg,ctrl_out),dim=1)
        
        read_vec = self.read_layer(comb)
        read_param = self._split(read_vec,'r')
        
        write_vec = self.write_layer(comb)
        write_param, e, a = self._split(write_vec,'w')
